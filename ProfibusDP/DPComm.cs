...
namespace FieldIot.ProfibusDP
{
    public class DPComm
    {
        public cParseGSD pGSD;

        public DPComm(System.IO.Ports.SerialPort sp, string fileBase)
        {
            serialPort1 = sp;
            gsdFileBase = fileBase;
            pGSD = new cParseGSD();
        }
        ...
        public List<CDEV_CFG_INFO> asLocalDevCfgList = new List<CDEV_CFG_INFO>();               //网络配置信息列表
        ...
        public void parseGSD(int devIndex, StreamWriter sw)
        {
            gsdFileName = asLocalDevCfgList[devIndex].gsdFileName;
            gsdFileName = gsdFileBase + gsdFileName;
            if (File.Exists(gsdFileName))
            {
                saveLogfile(sw, "GSD file {0} is found.", gsdFileName);

                StreamReader objReader = new StreamReader(gsdFileName);

                string strLine = "";
                strLine = objReader.ReadLine();
                string strLast = "";
                saveLogfile(sw, "GSD file {0} is opened.", gsdFileName);
                if (strLine != null)
                {
                    strLine = strLine.Trim();       //去除两端的空格（如果有的话）
                    ...
                    while (strLine != null)
                    {
                        pGSD.preProcessGSDStringInfo(strLine);
                ...
                //关闭记录文件
                saveLogfile(sw, "GSD file {0} is closed.", gsdFileName);
                objReader.Close();
            }
            else
            {
                saveLogfile(sw, "GSD file {0} is not found.", gsdFileName);
            }
        }

        public void clearDev()
        {
            asLocalDevCfgList.Clear();
        }
        ...
        //将接收缓冲区中的DP报文分成单个的报文
        //
        public void SimpleParseDPFrame(byte[] aucMsgData, ushort uiMsgLen, ref CFRAME_PARSE_NODE cFrameParseNode)
        {
            int i;
            byte ucFrameLen = 0;
                    ...
                    case SD1:
                        ...
                        if ((uiMsgWholeLen - i) >= ucFrameLen)
                        {
                            if (aucMsgWhole[i + 5] == 0x16)
                            {
                                ...
                                //处理完整报文
                                processSingleFrameForSerial(aucFrame, ucFrameLen, ref cFrameParseNode);
        ...
        //分析诊断数据
        //
        public void parseDiagnosticsInfo(byte[] aucDiagData, byte ucDataLen, ref string strDiagRlt)
        {
            strDiagRlt = "";
        ...
        //分析诊断数据
        //
        public string parseSlaveStateByDiag(byte[] aucDiagData, byte ucDataLen)
        {
            //WAIT_PRM
            if ((aucDiagData[0] & 0x02) == 0x02 &&
        ...
        //
        public bool processSingleFrameForSerial(byte[] aucFrame, byte ucFrameLen, ref CFRAME_PARSE_NODE cFrameParseNode)
        {
            //判断报文的长度不能超过最大长度
        ...
        public bool SetPRM()
        {
            ...
            aucData[3] = asLocalDevCfgList.ElementAt(0).ucMinTsdr;
            aucData[4] = asLocalDevCfgList.ElementAt(0).ucIdentH;
            aucData[5] = asLocalDevCfgList.ElementAt(0).ucIdentL;
            aucData[6] = 0;
            ucDataLen = 7;

            //生成SET_PRM报文                        
            build_DDLM_Set_Prm_Req(asLocalDevCfgList.ElementAt(0).ucAddrS, ucThisNode, ref aucFrame, ref ucFrameLen, aucData, ucDataLen,
                                    asLocalDevCfgList.ElementAt(0).aucUserExtPrmData, asLocalDevCfgList.ElementAt(0).ucUserExtPrmDataLen,
                                    ucFC_Byte);
            ...
            if (bytetoread > 0)
            {
                ...
                if (cFrameParse.strReqRsp == "Rsp")
                {
                    if (asLocalDevCfgList.ElementAt(0).strWK_State == "WAIT_PRM_SC")
                    {
                        //生成诊断报文
                        build_DDLM_Slave_Diag_Req(asLocalDevCfgList.ElementAt(0).ucAddrS, ucThisNode, ref aucFrame, ref ucFrameLen, ucFC_Byte);
                        ucFC_Byte = changeFC_FCB(ucFC_Byte);
                        ...
                        if (bytetoread > 0)
                        {
                            CFRAME_PARSE_NODE cFrameParse1 = new CFRAME_PARSE_NODE();

                            SimpleParseDPFrame(buff, (ushort)bytetoread, ref cFrameParse1);
                        }
                        if (cFrameParse.strReqRsp == "Rsp")
                        {
        ...
        public bool dataExchange(byte[] aucOutputData, byte ucOutputLen)
        {
            byte[] aucFrame = new byte[255];
            byte ucFrameLen = 0;
            //生成并发送DATA_EXCHANGE报文
            build_DDLM_Data_Exchange_Req(asLocalDevCfgList.ElementAt(0).ucAddrS, ucThisNode, ref aucFrame, ref ucFrameLen, aucOutputData, ucOutputLen, ucFC_Byte);
            ucFC_Byte = changeFC_FCB(ucFC_Byte);

            //发送报文
            sendFrame("Data_Exchange", aucFrame, ucFrameLen, asLocalDevCfgList.ElementAt(0).ucAddrS, false);
            byte[] buff = new byte[MAX_RCV_BUFFER_SIZE];
        ...
        public bool ChkCFG()
        {
            ...
            //生成并发送CHK_CFG报文
            build_DDLM_Chk_Cfg_Req(asLocalDevCfgList.ElementAt(0).ucAddrS, ucThisNode, ref aucFrame, ref ucFrameLen,
                                    asLocalDevCfgList.ElementAt(0).aucCfgData, asLocalDevCfgList.ElementAt(0).ucCfgDataLen,
                                    ucFC_Byte);
            ucFC_Byte = changeFC_FCB(ucFC_Byte);

            //发送报文
            sendFrame("Chk_Cfg", aucFrame, ucFrameLen, asLocalDevCfgList.ElementAt(0).ucAddrS, false);
            byte[] buff = new byte[MAX_RCV_BUFFER_SIZE];
            ...
            if (bytetoread > 0)
            {
                ...
                if (cFrameParse.strReqRsp == "Rsp")
                {
                    if (asLocalDevCfgList.ElementAt(0).strWK_State == "WAIT_CFG_SC")
                    {
                        //生成诊断报文
                        build_DDLM_Slave_Diag_Req(asLocalDevCfgList.ElementAt(0).ucAddrS, ucThisNode, ref aucFrame, ref ucFrameLen, ucFC_Byte);
                        ucFC_Byte = changeFC_FCB(ucFC_Byte);
        ...
        public bool diagDevice(byte ucDevIndex, ref CFRAME_PARSE_NODE cFrameParse)
        {
            //生成诊断报文
            byte[] aucFrame = new byte[255];
            byte ucFrameLen = 0;
        ...
        public byte changeFC_FCB(byte ucFC)
        {
            byte ucFC_Byte = ucFC;
